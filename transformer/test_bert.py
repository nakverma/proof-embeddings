...
FINETUNED_LOAD_PATH = os.path.join('saved_models', 'finetuned', 'step_classifier.pt')
...
FEATURE_EXTRACTION_METHODOLOGY = 'pooled'
EVALUATION_METHODOLOGY = 'lmnn+knn'
GENERATE_DATA = False
EVALUATE = False
...
UNSUPERVISED_ANOMALY_DETECTION = True
...
test_dataset = LogicDataset(data_path=DATA_PATH,
                            ...
                            data_mode='pairs',
                            seed=SEED)
print('NUM LABELS: ', test_dataset.get_num_labels())
print('Labels: ', test_dataset.get_labels())
...
model.load_state_dict(torch.load(FINETUNED_LOAD_PATH, map_location=DEVICE))
...
if GENERATE_DATA:
    ...
    for batch in tqdm(test_loader, desc='Extracting Features for Testing Data'):
        ...
        sequence_output, pooled_output = x_[0], x_[1]
    ...
    if EVALUATION_METHODOLOGY == 'lmnn+knn':
        lmnn = LargeMarginNearestNeighbor(n_neighbors=3, max_iter=50, n_components=X_train.shape[-1], verbose=1, n_jobs=-1)
...
if COMPUTE_ATTENTION:
    ...
    df = df.set_index('Law')
    df.to_csv(os.path.join('data', 'law_attentions.csv'))
...
del train_loader, train_dataset, test_loader, model, config
